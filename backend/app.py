...
REDIRECT_URI = "https://stayed-materials-tank-fill.trycloudflare.com/facebook/callback"
OAUTH_LINK = f"https://www.facebook.com/v14.0/dialog/oauth?client_id={FB_APP_ID}&redirect_uri={REDIRECT_URI}&scope=pages_show_list,pages_read_engagement,pages_messaging,pages_manage_metadata&response_type=code"
...
def get_name_from_messages(conversation_id, access_token):
...
async def test_send(user_id: str):
    """ทดสอบส่งข้อความ"""
    print(f"🧪 Testing send message to: {user_id}")
    result = send_message(user_id, "🧪 ทดสอบการส่งข้อความ - Test Message")
    return {"message": f"ส่งข้อความทดสอบไปยัง {user_id} แล้ว", "result": result}
